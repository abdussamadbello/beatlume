    ...
    try:
        async with session_maker() as db:
            ...
            result = await db.execute(
                select(Scene).where(Scene.story_id == uuid.UUID(story_id)).order_by(Scene.n)
            )
...
async def _run_generate_insights(task_id: str, story_id: str, org_id: str) -> dict:
    ...
    try:
        async with session_maker() as db:
            ...
            scenes = list(
                (await db.execute(select(Scene).where(Scene.story_id == uuid.UUID(story_id))))
                .scalars()
                .all()
            )
            ...
            result = await run_graph("insight", graph, state)
            if result.get("error"):
...
async def _run_apply_insight(task_id: str, story_id: str, org_id: str, insight_id: str) -> dict:
    engine, session_maker = await _with_session()
    try:
        async with session_maker() as db:
            await _set_org_rls(db, org_id)
            in_id = uuid.UUID(insight_id)
            insight_row = await insight_service.get_insight(db, uuid.UUID(story_id), in_id)
            ...
            for n in allowed_list:
                sc_r = await db.execute(
                    select(Scene).where(Scene.story_id == uuid.UUID(story_id), Scene.n == n)
                )
            ...
            if not scene_blocks:
                raise ValueError("No matching scenes for this insight’s scene numbers.")
            ...
            result = await call_llm("insight_apply", messages, temperature=0.35, max_tokens=4000)
            if not result:
            ...
            if total_applied < 1:
                raise ValueError("No changes could be applied. Check scene numbers or try again.")

            ok = await insight_service.dismiss_insight(db, uuid.UUID(story_id), in_id)
            if not ok:
    ...
    try:
        async with session_maker() as db:
            ...
            drafts = list(
                (
                    await db.execute(
                        select(DraftContent).where(DraftContent.story_id == uuid.UUID(story_id))
                    )
                )
                .scalars()
                .all()
            ...
            nodes = list(
                (
                    await db.execute(
                        select(CharacterNode).where(CharacterNode.story_id == uuid.UUID(story_id))
                    )
                )
                .scalars()
                .all()
            ...
            edges = list(
                (
                    await db.execute(
                        select(CharacterEdge).where(CharacterEdge.story_id == uuid.UUID(story_id))
                    )
                )
                .scalars()
                .all()
            ...
            graph_result = await run_graph(
                "relationship", graph, {"pairs": pairs, "results": [], "error": None}
            )
            ...
            for pair_input, item in zip(pairs, results):
                ...
                existing = edge_by_pair.get((source_node_id, target_node_id)) or edge_by_pair.get(
                    (target_node_id, source_node_id)
                )
                shared_refs = [
                    s.get("n") for s in pair_input["shared_scenes"] if s.get("n") is not None
                ]
                evidence = [
                    {
...
async def _run_summarize_scene(
    ...
    try:
        async with session_maker() as db:
            ...
            pov = await assembler.retriever.get_character_by_name(uuid.UUID(story_id), scene.pov)
            pov_character = None
            if pov is not None:
            ...
            result = await run_graph("summary", graph, state)
            if result.get("error"):
                raise ValueError(result["error"])
...
@celery_app.task(bind=True, max_retries=3, retry_backoff=True, rate_limit="10/m")
...
@celery_app.task(bind=True, max_retries=2, retry_backoff=True, rate_limit="10/m")
def apply_insight(self, story_id: str, org_id: str, insight_id: str):
    ...
    try:
        return asyncio.run(_run_apply_insight(self.request.id, story_id, org_id, insight_id))
    except Exception as exc:
        publish_event(
            story_id,
            "ai.error",
