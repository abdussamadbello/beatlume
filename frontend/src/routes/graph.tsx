...
function GraphView() {
  ...
  const visibleNodes = useMemo(() =>
    filters.hideMinor ? scaledNodes.filter(n => n.type !== 'minor') : scaledNodes,
    [scaledNodes, filters.hideMinor]
  )

  const selectedNode = selectedNodeId ? nodes.find(n => n.id === selectedNodeId) : null
  const selectedEdges = selectedNodeId
    ? edges.filter(e => e.a === selectedNodeId || e.b === selectedNodeId)
    : []
            ...
            <div style={{ marginTop: 'auto', borderTop: '1px dashed var(--line)', paddingTop: 10 }}>
              ...
              {selectedNode ? (
                <div style={{ fontSize: 11, color: 'var(--ink-2)', marginTop: 4 }}>
                  {selectedEdges.length} relationships {'\u00B7'} {selectedNode.type === 'hub' ? 'hub node' : 'standard node'}
                </div>
              ) : (
